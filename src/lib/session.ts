...
export function getSession(sessionId: string): SessionRecord | null {
  const row = db.prepare('SELECT session_id as sessionId, user_id as userId, created_at as createdAt FROM sessions WHERE session_id = ?')
    .get(sessionId);
  return row ?? null;
}
...
export function getUserIdFromCookieValue(sessionId: string | null): string | null {
  if (!sessionId) return null;
